...
public class BTree<K extends Comparable<K>, V> {
    ...
    public void put(K k, V val) {
        ...
        Node u = insert(root, k, val, height);
        n++;
        if (u == null) return;
    ...
    private Node insert(Node h, K k, V val, int ht) {
        int j;
        Entry t = new Entry(k, val, null);

        // external node
        if (ht == 0) {
            for (j = 0; j < h.m; j++) {
                if (less(k, h.children[j].key)) break;
            }
        }
        ...
        // internal node
            ...
            for (j = 0; j < h.m; j++) {
                if ((j + 1 == h.m) || less(k, h.children[j + 1].key)) {
                    Node u = insert(h.children[j++].next, k, val, ht - 1);
