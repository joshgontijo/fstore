...
public class Midpoints<K extends Comparable<K>> {
    ...
    public void writeTo(FooterWriter writer, Codec codec, int blockSize, BufferPool bufferPool, Serializer<K> keySerializer) {
        ...
        AtomicInteger blockIdx = new AtomicInteger();
        entries.sort(Comparator.comparing(o -> o.key));
        Blocks.writeAll(entries, block, bufferPool, serializer, b -> {
            writer.write(BLOCK_PREFIX + blockIdx.getAndIncrement(), block, blockSerializer);
        });
    }
