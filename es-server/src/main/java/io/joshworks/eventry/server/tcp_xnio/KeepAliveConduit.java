...
public class KeepAliveConduit implements StreamSinkConduit, StreamSourceConduit {
    ...
    protected void doSend() {
        try {
            ...
            byte[] bytes = "aaaaaaaaaaaaaaaaaaa".getBytes(StandardCharsets.UTF_8);
            var bb = ByteBuffer.allocate(Integer.BYTES + bytes.length).putInt(bytes.length).put(bytes).flip();
            sink.write(bb);
    ...
    public KeepAliveConduit(StreamConnection connection, int interval) {
