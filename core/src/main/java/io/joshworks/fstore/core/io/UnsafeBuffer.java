...
public class UnsafeBuffer {
    ...
    public static void absolutePut(ByteBuffer dstBuffer, int dstPosition, ByteBuffer srcBuffer, int srcPosition, int count) {
        int bytesToCopy = Math.min(count, srcBuffer.remaining());
        if (!srcBuffer.isDirect()) {
            absolutePut(dstBuffer, dstPosition,
                    srcBuffer.array(), srcBuffer.arrayOffset() + srcPosition,
                    bytesToCopy);
            return;
        }
        ...
        if (UNSAFE != null && ADDRESS_FIELD != null && dstBuffer.isDirect()) {
            try {
                long dstAddress = (long) ADDRESS_FIELD.get(dstBuffer) + dstPosition;
                long srcAddress = (long) ADDRESS_FIELD.get(srcBuffer) + srcPosition;
                UNSAFE.copyMemory(srcAddress, dstAddress, bytesToCopy);
            } catch (IllegalAccessException e) {
                throw new RuntimeException(e);
            }
        } else {
            // fallback to basic loop
            for (int i = srcPosition; i < srcBuffer.limit(); i++) {
                dstBuffer.put(dstPosition + i, srcBuffer.get(i));
            }
        }
    }
